package com.graydang.app.batch.bill.jobs.update.promulgation.processor;
...
public class BillStatusHistoryPromulgationItemProcessor implements ItemProcessor<Bill, BillStatusHistoryPromulgationUpdateDto> {
     ...
     * @return '공포' 정보가 확인되면 Writer로 넘길 DTO, 없으면 null (필터링)
     */
    @Override
    public BillStatusHistoryPromulgationUpdateDto process(Bill bill) throws Exception {
        try {
            // 1. API 호출
            Optional<BillPromulgationInfoResponseDto> responseOpt = billApiClient.getBillPromulgationInfo(bill.getBillId());
            ...
            // 3. 유효한 데이터 처리
            BillPromulgationInfoResponseDto.PromulgationItem item = responseOpt.get().getBody().getItems().get(0);
            return new BillStatusHistoryPromulgationUpdateDto(bill.getId(), bill.getBillId(), item);
        } catch (Exception e) {
            // 4. 에러 처리 전략
    ...
    // 검증 로직 분리
    private boolean shouldSkip(Optional<BillPromulgationInfoResponseDto> responseOpt) {
        ...
        return item.getAnounceDt() == null || item.getAnounceDt().isBlank();
    }
}
